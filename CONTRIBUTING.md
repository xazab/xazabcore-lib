# Contributing to xazabcore-lib

We're working hard to make *xazabcore-lib* the most powerful JavaScript library for working with Xazab. Our goal is to have *xazabcore-lib* be a library that can be used by anyone interested in Xazab, and to level expertise differences with great design and documentation.
...
These are some global design goals in xazabcore-lib that any change must adhere.
...
When possible, xazabcore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of xazabcore-lib methods. For example, calling `JSON.stringify` on an xazabcore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'xazabcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the xazabcore object.

DO:
```javascript
var xazabcore = require('../');
var PublicKey = xazabcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add xazab git@github.com:xazab/xazabcore-lib.git
git pull --rebase xazab master
```
...
```
Finally go to [github.com/xazab/xazabcore-lib](https://github.com/xazab/xazabcore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of xazabcore-lib.

If you have any questions feel free to post them to
[github.com/xazab/xazabcore-lib/issues](https://github.com/xazab/xazabcore-lib/issues).
